...
public class InputStreamTest {
	public static void main(String[] args)  throws IOException{
		...
		//readerTest();
		//textFileCopy();
		streamfileCopy(); //fileinputStream, fileoutputStream //파일복사
	}
	...
	public static void streamfileCopy() throws IOException{
		BufferedInputStream fis = new BufferedInputStream(new FileInputStream(new File("Chrysanthemum.jpg")));
		BufferedOutputStream fos = new BufferedOutputStream(new FileOutputStream(new File("국화.jpg")));
		int size;
		byte[] b1 = new byte[1000];
//		int b1;
		while((size = fis.read(b1)) != -1) {
			fos.write(b1, 0, size);
		}
		fos.close();
		fis.close();
